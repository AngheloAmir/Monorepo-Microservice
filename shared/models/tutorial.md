...
- A **prisma.config.ts** file to configure the database connection for migrations and the studio.
...
## 6. Example Usage with Supabase

When using Supabase (or any other provider using PgBouncer for connection pooling), you need to configure Prisma to use two different connection strings:
1. **Transaction Pooler URL** (`DATABASE_URL`): For your application to run queries. This manages connections efficiently in serverless environments.
2. **Session/Direct URL** (`DIRECT_URL`): For migrations and schema overrides. This bypasses the pooler to perform schema changes.

### Step 1: Update .env

Add both URLs to your `.env` file. You can find these in your Supabase Dashboard under Project Settings > Database.
...
You need to update `shared/models/prisma.config.ts` to use the `directUrl` property in the `datasource` object.
...
Once configured, the migration workflow is identical to the standard workflow. Prisma automatically uses the `DIRECT_URL` for migrations because of the `directUrl` field in your schema.

```bash
# Apply changes to the Supabase database
cd shared/models
npm run db:migrate
```

*Note: If you haven't set up `DIRECT_URL` in your schema, `db:migrate` might fail or hang when connected to a pooler.*
